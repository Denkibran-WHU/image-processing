...
class Functions:
    @staticmethod
    def GetClockAngle(v1, v2): # 计算2个向量模的乘积 ,返回夹角
        TheNorm = np.linalg.norm(v1)*np.linalg.norm(v2)# 叉乘
        rho = np.rad2deg(np.arcsin(np.cross(v1, v2)/TheNorm)) # 点乘
    ...
    @staticmethod
    def couputeMean(deg):#对数据进行处理，提取均值
...
class MeterDetection:
    def __init__(self,path):
        ...
        self.outputPath=('outputs/') # self.outputPath=(os.getcwd()+'/outputs/').replace('\\','/')
        ...
        self.r=None                 #半径
        self.divisionValue=4/1800  #分度值
        self.makeFiledir()
        self.markZeroPoint()


    def makeFiledir(self): #创建输出文件夹
        if not os.path.exists(self.outputPath):   # 是否存在这个文件夹
            os.makedirs(self.outputPath)          # 如果没有这个文件夹，那就创建一个

    def markZeroPoint(self):                      #弹出图片窗口 人工标记起始点0点的位置
        img =self.image
    ...
    def ImgCutCircle(self):    #截取表盘区域，滤除背景
        img=self.image
    ...
    def ContoursFilter(self):   #对轮廓进行筛选

        r_1, c_x, c_y = self.cirleData

        img = self.panMask.copy()
        # cv2.circle(img, (c_x, c_y), 20, (23, 28, 28), -1)
        img = cv2.GaussianBlur(img, (3, 3), 0)                                                #高斯滤波 GaussianBlur
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)                                          #灰度转换
        binary = cv2.adaptiveThreshold(~gray, 255,
                                    cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 3, 0) #自适应二值化adaptiveThreshold

        if cv2.__version__ >'4.0.0':
            contours, hier = cv2.findContours(binary, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
        else:
            aa,contours, hier = cv2.findContours(binary, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)  #概率霍夫圆检测
        ...
        for cnt in contours:
            ...
            # print(rect)
            #（中心点坐标，（宽度，高度）,旋转的角度）=（a,(w,h),c )  = rect
            a, (w, h), c = rect
            w = int(w)
            h = int(h)
            ''' 满足条件:“长宽比例”，“面积”'''
    ...
    def FitNumLine(self): #将筛选出的矩形拟合成直线
    ...
    def getIntersectionPoints(self):  #获取刻度线交点
    ...
    def FitPointerLine(self):  #拟合指针直线段
    ...
    def Readvalue(self):  #返回计算值
        try:
            ...
            readValue=self.divisionValue*theta  #刻度值为夹角乘以分度值
            readValue+=0.02                    #由于表盘不均匀，设定偏移量
